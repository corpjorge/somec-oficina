<template>
    <div class="col-lg-12 mx-auto p-3 py-md-5">
        ...
        <div v-if="errors.correo" class="alert alert-danger" role="alert">
            {{ errors.correo[0] }}
        </div>

        <form class="row g-3" @submit.prevent="crear">
            <div class="col-auto">
                <label for="evento" class="visually-hidden">Nombre nuevo evento</label>
                <input type="text" class="form-control" id="evento" placeholder="Nombre nuevo evento"
                       v-model="evento.nombre" required>
            </div>
            <div class="col-auto">
                <label for="correo" class="visually-hidden">Correo responsable</label>
                <input type="text" class="form-control" id="correo" placeholder="Correo responsable"
                       v-model="evento.correo" required>
            </div>
            <div class="col-auto">
            ...
            <div class="col-auto">
                <div v-if="enviando" class="alert alert-success alert-dismissible fade show" role="alert"
                     style="padding-bottom: 4px; padding-top: 2px;">
                    Evento creado
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"
                            style="padding-top: 2px; padding-bottom: 14px;" @click="close"></button>
                </div>
            </div>
                    ...
                    <tr v-for="dato in datos" :key="dato.id">
                        ...
                        <td>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal"
                                                    @click="eliminarEvento(dato.id)">Eliminar
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <router-link type="button" class="btn btn-success btn-sm"
                                         :to="{ name: 'Editar', params: { evento: dato.nombre } }">
...
export default {
    ...
    methods: {
        ...
        async obtenerDatos() {
            await axios.get('/admin/eventos/datos').then(response => {
        ...
        async eliminarEvento(id) {
            await axios.delete('/admin/eventos/eliminar/' + id).then(() => {
                this.obtenerDatos();
            })
        },
        async crear() {
            if (!this.evento.correo) {
                return this.errors.correo = "El correo no puede estar vacío";
            }

            const re = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i

            if (!re.exec(this.evento.correo)) {
                return this.errors = {correo: {0: 'No es un correo valido'}}
            }

            this.errors.correo = false;

            await axios.post('/admin/eventos/crear', this.evento).then(() => {
                this.enviando = true;
                this.obtenerDatos();
                this.evento = {}
            }).catch(error => {
