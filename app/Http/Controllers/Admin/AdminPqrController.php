...
use App\Mail\PQRSAntendidoCerrado;
...
class AdminPqrController extends Controller
{
    ...
    public function atender(Request $request, $id)
    {
        ...
        if ($request->file('archivo_antendido')) {
        ...
        Mail::to($pqrs->correo)->send(new PQRSAntendidoCerrado($request->observacion_antendido, 'ATENDIDO', $archivo1 ?? ''));
    }
    ...
    public function cerrar(Request $request, $id)
    {
        ...
        if ($request->file('archivo_cierre')) {
        ...
        Mail::to($pqrs->correo)->send(new PQRSAntendidoCerrado($request->observacion_cierre, 'CERRADO', $archivo1 ?? ''));
    }
}
